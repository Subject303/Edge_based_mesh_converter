...
if filename is None :
    ...
    preprocceced_dest = './preprocessed_mesh_folder/'+filename[:-4]+'.preprocessed_mesh_file'
...
algo.SetAbsoluteTolerance(1e-8)
...
for c in range(nele):
    ...
    for f in range(cell.GetNumberOfFaces()): 
        ...
        f_p_obj_relation_array.append(list(dict.fromkeys([face.GetPointId(p) for p in range(face.GetNumberOfPoints())])))
