...
!             print*,'aa'
    ...
    subroutine boundary_face_volume_processing
        ...
        integer(KIND=INT32) :: k, pf, edge_count_real, face_count_real, flag, remainder_count
        integer(KIND=INT32),allocatable :: centroid_index_array(:)
        real(KIND=REAL64),allocatable   :: centroid_array(:,:)
        ...
        do bp = 1, b_npoin
            ...
            if (feature_points(bp)) then
                ...
                do pe=(pe_start+1),pe_end
                    print*, pe, p_e_obj_relation_array(pe),reversed_e_bound_indexing_array(p_e_obj_relation_array(pe))
